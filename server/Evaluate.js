...
const output = fs.readFile('./data/output.json').then(e=>JSON.parse(e.toString()))
const mainFive = fs.readFile('./data/mainFive.json').then(e=>JSON.parse(e.toString()))
const allFive = fs.readFile('./data/allFive.json').then(e=>JSON.parse(e.toString()))
const mainFour = fs.readFile('./data/mainFour.json').then(e=>JSON.parse(e.toString()))
...
module.exports = async (tried, col) => {
    ...
    const date = new Date().toDateString().split(' ')
    const day = date[1] + date[2]
    let today = await fs.readFile('./data/todaysWord.json').then(e=>JSON.parse(e.toString()))
    today = today[day]
    let chosenWord;
